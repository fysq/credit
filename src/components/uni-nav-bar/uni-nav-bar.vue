<template>
	<view class="uni-navbar" :class="{ 'uni-dark': dark, 'uni-nvue-fixed': fixed }">
		<view class="uni-navbar__content"
			...
			<view :style="{ color: themeColor, height: navbarHeight }" class="uni-navbar__header">
				<view @tap="onClickLeft" class="uni-navbar__header-btns uni-navbar__header-btns-left"
					:style="{ width: leftIconWidth }" v-if="type == 0">
					<slot name="left">
				...
				<view class="uni-navbar__header-btns uni-navbar__header-btns-left" :style="{ width: leftIconWidth }"
					v-if="type == 1">
					<view class="customNavBarLeft" :class="{ 'vn': tabPage }">
						<view class="cl-inner">
							<view class="cl-btn back" @tap="onClickLeft1"></view>
							<view class="cl-btn home" @tap="onClickLeft2"></view>
						</view>
					</view>
				</view>
				<view class="uni-navbar__header-container">
					<slot>
						<!--这里是正常状态-->
						<view class="uni-navbar__header-container-inner" v-if="title.length > 0 && type == 0">
							<text class="uni-nav-bar-text uni-ellipsis-1" :style="{ color: themeColor }">{{ placeholder ?
								title : '' }}</text>
						</view>
						<!--这里是变换状态-->
						<view class="uni-navbar__header-container-inner" v-if="title.length > 0 && type == 1">
		...
		<!-- #ifndef APP-NVUE -->
		<view class="uni-navbar__placeholder" v-if="fixed && placeholder" :style="{ 'background': themeBgColor }">
			<status-bar v-if="statusBar" />
...
export default {
	...
	computed: {
		themeBgColor() {
			...
			return this.placeholder ? this.backgroundColor || '#FFF' : 'transparent';
		},
		themeColor() {
			if (this.dark) {
				// 默认值
	...
	methods: {
		...
		onClickLeft1() {
			...
			this.$emit("clickLeft1");
		},
		onClickLeft2() {
			this.$Router.pushTab({
				name: 'index'
			})
			this.$emit("clickLeft2");
		},
		onClickRight() {
