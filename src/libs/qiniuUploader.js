...
function randomKey(fileName){
  let newFile = moment().format('YYYY/MM/DD/HHmmss') + Math.floor(Math.random() * 1e6) + '/' +  fileName;
  return newFile;
}
...
function doUpload(filePath, success, fail, options, progress, cancelTask) {
    ...
    if (!config.qiniuShouldUseQiniuFileName) {
      formData['key'] = randomKey(fileName);
    }
